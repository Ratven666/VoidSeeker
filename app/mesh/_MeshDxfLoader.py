...
class MeshDxfLoader:
    ...
    def init_mesh_from_dxf(self, file_path: str):
        ...
        # Создаем Open3D mesh
        self.base_mesh.mesh = o3d.geometry.TriangleMesh()
        self.base_mesh.mesh.vertices = o3d.utility.Vector3dVector(np.array(vertices, dtype=np.float64))
        self.base_mesh.mesh.triangles = o3d.utility.Vector3iVector(np.array(faces, dtype=np.int32))

        # Для корректной работы базового класса создаем облако точек из вершин меша
        self.base_mesh.pcd = o3d.geometry.PointCloud()
        self.base_mesh.pcd.points = self.base_mesh.mesh.vertices
